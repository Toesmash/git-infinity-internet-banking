...
import getCardTransactions from '../selectors/cardTransactions';
    ...
    return (
      <div>
        ...
        <div className="transactions__list">
          {transactions.length === 0 ? <p className="italic">Žiadne kartové transakcie.</p> : renderTxns}
        </div>
...
const mapStateToProps = (reduxStore) => {
  const card = (history.location.pathname).split('/').splice(1)[1];
  return {
    transactions: getCardTransactions(reduxStore.transactions, card, reduxStore.filter),
